...
## Google Play / App Store 同期の使い方

- ゲーム詳細にて、対象プラットフォームのエントリが存在すると同期ボタンが表示されます。
  - Google Play: 「Google Play 同期」
  - App Store: 「App Store 同期」
- 内部的に以下をコールします。
  - `POST /projects/:project_id/games/:id/sync?platform=google_play`（直近30日）
  - `POST /projects/:project_id/games/:id/sync?platform=app_store`（直近30日）

## 資格情報の入力

- プロジェクト > ゲーム > プラットフォーム から各プラットフォームを追加/編集し、フォームのJSON欄に資格情報を貼り付けます。
  - Google Play: `google_play_credentials_json`（空可）
  - App Store: `apple_store_credentials_json`（空可, キー: `key_id`, `issuer_id`, `private_key`）
  - 空欄の場合は資格情報なしでモッククライアントが動作します。

## 保存先

- 同期結果は `PlatformDailyMetric` に保存されます。
  - 共通項目: `metric_date`, `installs`, `revenue_cents`, `average_rating`, `rating_count`, `country(nil)`
  - 一意性: `[:game_platform_id, :metric_date, :country]`
```
